...
public class Deaths {
    public static void firstWin(Player p){
        ...
        try {
            if (!exists(p)) {
                PreparedStatement ps = connection.prepareStatement("INSERT into UHCdeaths(uuid, points)\nvalues('" + p.getUniqueId().toString() + "', '" + number + "');");
                ps.executeUpdate();
                ps.close();
            }
        }catch(Exception e){
    ...
    public static boolean exists(Player p){
        try{
            PreparedStatement ps = connection.prepareStatement("SELECT uuid FROM UHCdeaths");
    ...
    public static void addWins(Player p){
        try{
            PreparedStatement ps = connection.prepareStatement("UPDATE UHCdeaths SET points= points+1 WHERE uuid='" + p.getUniqueId().toString() + "'");
            ps.executeUpdate();
            ps.close();
        }catch(Exception e){
            e.printStackTrace();
